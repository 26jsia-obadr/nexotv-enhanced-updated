...
const env = {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // CACHE CONFIGURATION (In-Memory LRU + SQLite)
    // ═══════════════════════════════════════════════════════════════════════
    CACHE_ENABLED: (process.env.CACHE_ENABLED || 'true').toLowerCase() !== 'false',
    CACHE_TTL_MS: parseInt(process.env.CACHE_TTL_MS || '86400000', 10), // 24 hours
    MAX_CACHE_ENTRIES: parseInt(process.env.MAX_CACHE_ENTRIES || '300', 10),
    SQLITE_PATH: process.env.SQLITE_PATH || null,
    SQLITE_GC_INTERVAL_MS: parseInt(process.env.SQLITE_GC_INTERVAL_MS || '21600000', 10), // 6 hours
    SQLITE_VACUUM_INTERVAL_MS: parseInt(process.env.SQLITE_VACUUM_INTERVAL_MS || '604800000', 10), // 7 days

    // ═══════════════════════════════════════════════════════════════════════
    // PREFETCH & LOGO CACHING
    // ═══════════════════════════════════════════════════════════════════════
    PREFETCH_ENABLED: (process.env.PREFETCH_ENABLED || 'true').toLowerCase() !== 'false',
    PREFETCH_MAX_BYTES: parseInt(process.env.PREFETCH_MAX_BYTES || '150000000', 10), // 150 MB
    LOGO_CACHE_ENABLED: (process.env.LOGO_CACHE_ENABLED || 'true').toLowerCase() !== 'false',

    // ═══════════════════════════════════════════════════════════════════════
    // ADDON METADATA
    // ═══════════════════════════════════════════════════════════════════════
    ADDON_NAME: process.env.ADDON_NAME || 'NexoTV-Enhanced',
    ...
    IP_RATE_LIMIT_WINDOW_MS: parseInt(process.env.IP_RATE_LIMIT_WINDOW_MS || '300000', 10), // 5 minutes
    ...
    TOKEN_RATE_LIMIT_WINDOW_MS: parseInt(process.env.TOKEN_RATE_LIMIT_WINDOW_MS || '60000', 10), // 1 minute
    TOKEN_RATE_LIMIT_MAX: parseInt(process.env.TOKEN_RATE_LIMIT_MAX || '60', 10),

    // ═══════════════════════════════════════════════════════════════════════
    // TMDB API (for enhanced metadata like posters, ratings)
    // ═══════════════════════════════════════════════════════════════════════
    TMDB_API_KEY: process.env.TMDB_API_KEY || null, // Optional global fallback
    ...
    EPG_UPDATE_INTERVAL_MS: parseInt(process.env.EPG_UPDATE_INTERVAL_MS || '') || 28800000, // 8 hours
    EPG_MAX_BYTES: parseInt(process.env.EPG_MAX_BYTES || '104857600', 10), // 100 MB

    // ═══════════════════════════════════════════════════════════════════════
    // DATA PROVIDERS (M3U, Xtream, IPTV-ORG)
    // ═══════════════════════════════════════════════════════════════════════
    IPTV_ORG_CACHE_TTL_MS: parseInt(process.env.IPTV_ORG_CACHE_TTL_MS || '') || 86400000, // 24 hours
    M3U_CACHE_TTL_MS: parseInt(process.env.M3U_CACHE_TTL_MS || '') || 86400000, // 24 hours
    DATA_MEMORY_TTL_MS: parseInt(process.env.DATA_MEMORY_TTL_MS || '300000', 10), // 5 minutes
    UPDATE_INTERVAL_MS: parseInt(process.env.UPDATE_INTERVAL_MS || '14400000', 10), // 4 hours
    MIN_UPDATE_INTERVAL_MS: parseInt(process.env.MIN_UPDATE_INTERVAL_MS || '1800000', 10), // 30 minutes
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // FETCH TIMEOUTS (different operations have different timeout thresholds)
    // ═══════════════════════════════════════════════════════════════════════
    FETCH_TIMEOUT_MS: parseInt(process.env.FETCH_TIMEOUT_MS || '30000', 10), // General HTTP requests
    EPG_FETCH_TIMEOUT_MS: parseInt(process.env.EPG_FETCH_TIMEOUT_MS || '60000', 10), // EPG downloads (larger files)
    PREFETCH_TIMEOUT_MS: parseInt(process.env.PREFETCH_TIMEOUT_MS || '45000', 10), // Background prefetch
    LOGO_TIMEOUT_MS: parseInt(process.env.LOGO_TIMEOUT_MS || '10000', 10), // Logo fetches
